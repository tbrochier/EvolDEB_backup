...
import util.MetaFilenameFilter;
...
///////////////////////////////
     ...
     *
     * PISCES :
     * float NCHL(time, s_rho, eta_rho, xi_rho) ;
		NCHL:long_name = "averaged Chlorophyll in nano" ;
		NCHL:units = "mg Chl m-3" ;
		NCHL:field = "NCHL, scalar, series" ;
	float DCHL(time, s_rho, eta_rho, xi_rho) ;
		DCHL:long_name = "averaged Chlorophyll in diatoms" ;
		DCHL:units = "mg Chl m-3" ;
		DCHL:field = "DCHL, scalar, series" ;
    ...
    public static String directory_roms,  sufixe, directory_Suplementary_data,oxyclin_filename,PonteOBS_filename,PonteCLM_filename;
    static double[][] pm,  pn;
    ...
    public static util.Lire_configuration param_config;
///////////////////////////////
...
//        String fileName = directory_roms  + "NCML/NCML_per_MONTH/union_plankton_Y" + year + "M" +month + ".ncml";
//        String fileName = "/Volumes/SIMU_PEDRO/SENEGAL/union_plankton.ncml";
//        String fileName = "/Users/timbrochier/Desktop/tmp/TEST/union_plankton_Y" + year +"M" +month + ".ncml";
    ...
    public void init() throws IOException {
        ...
        // open(getFile(t0));
        String fileName = get_filename(Simulation.year, 1);
        open(fileName);
        ...
        // Si un seul pas de temps par fichier mensuel, il faut lire le 1ire le 1er pas de temps du mois suivant...
            // Par mesure de simplification dans ce cas, on fixe directement dt_HyMo = 30 jours
        if (nbTimeRecords>1){
        get_dt_HyMo();
        } else {
        dt_HyMo = 30; //(30 jours)
        ...
        if (ncIn.findVariable(strSal) == null) {
            throw new IOException(
                    "Salinity field " + strSal + " not found in file " + ncIn.getLocation());
        } else {
            FLAG_SAL = true;
        }
        ...
        FLAG_PLANKTON_PISCES = !Simulation.FLAG_PLANKTON_INTEG;
...
if(FLAG_PLANKTON_PISCES){
        if (ncIn.findTopVariable(strMicroZoo) == null || ncIn.findTopVariable(strNanoPhyto) == null || ncIn.findTopVariable(strDiatoms) == null) {
                throw new IOException(
                        "Cannot implement growth limitation. PISCES Plankton fields" + " not found in file " + ncIn.getLocation());
            } else {
                FLAG_PLANKTON_PISCES = true;
            //}
        }
        }
  
if(FLAG_PLANKTON_NPZD){
            if (ncIn.findTopVariable(strPhyto) == null || ncIn.findTopVariable(strZoo) == null ) {
                throw new IOException(
                        "Cannot implement growth limitation. NPZD Plankton fields" + " not found in file " + ncIn.getLocation());
            } else {
                FLAG_PLANKTON_NPZD = true;
        }
}

        /** plankton concentration */
        if (Simulation.flagRET_Chla_PISCES ){//|| Simulation.flagSwim_O2) { // rétablir si lecture O2 Pisces necessaire pour swim (retablir dans Poissons et dans Dataset_EVOL)
            if (ncIn.findTopVariable(strDiatomsChl) == null || ncIn.findTopVariable(strNanoPhytoChl) == null) {
                throw new IOException(
                        "Cannot get Chla fields" + " not found in file " + ncIn.getLocation());
                }
        }
...
//        time_tp1 = t0;
    ...
    private void get_dt_HyMo() {
        ...
        try {
//            ucar.ma2.ArrayFloat.D1 xTimeTp1 = (ucar.ma2.ArrayFloat.D1) ncIn.findVariable(strTime).read();
        Array xTimeTp1 = ncIn.findVariable(strTime).read();//            time_tp1 = xTimeTp1.getFloat(xTimeTp1.getIndex().set(0)); //xTimeTp1.getFloat(xTimeTp1.getIndex().set(0));
...
//            double [] time_tp1_tmp = (double[]) xTimeTp1.copyTo1DJavaArray(); //xTimeTp1.getFloat(xTimeTp1.getIndex().set(0));
    ...
    static void setAllFieldsTp1AtTime(int rank) throws IOException {
        ...
        try {
            ...
            if (FLAG_W) {
                ...
                w_tp1 = (float[][][]) ncIn.findVariable(strW).read(origin,
            ...
            if (FLAG_HBL && Simulation.lire_HBL) {
                hbl_tp1 = (float[][]) ncIn.findVariable(strHBL).read(origin,
                        new int[]{1, ny, nx}).reduce().copyToNDJavaArray();
            }
            ...
            if (FLAG_PLANKTON_PISCES) {
                Diatoms_tp1 = (float[][][]) ncIn.findVariable(strDiatoms).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                NanoPhyto_tp1 =
                        (float[][][]) ncIn.findVariable(strNanoPhyto).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                MicroZoo_tp1 =
                        (float[][][]) ncIn.findVariable(strMicroZoo).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                MesoZoo_tp1 =
                        (float[][][]) ncIn.findVariable(strMesoZoo).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
...
//                O2_tp1 =
  //                      (float[][][]) ncIn.findVariable(strO2).
      ...
      //                  copyToNDJavaArray();
            ...
            if (FLAG_PLANKTON_NPZD) {
                Phyto_tp1 = (float[][][]) ncIn.findVariable(strPhyto).
                        ...
                        copyToNDJavaArray();
                Zoo_tp1 =
                        (float[][][]) ncIn.findVariable(strZoo).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                Det_tp1 =
                        (float[][][]) ncIn.findVariable(strDet).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                NO3_tp1 =
                        (float[][][]) ncIn.findVariable(strNO3).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();
                Chla_tp1 =
                        (float[][][]) ncIn.findVariable(strCHLA).
                        read(origin, new int[]{1, nz, ny, nx}).reduce().
                        copyToNDJavaArray();            }
            

            if (Simulation.FLAG_PLANKTON_INTEG) {
                Diatoms_integ_tp1 = (float[][]) ncIn.findVariable(strDiatoms_int).
                        read(origin, new int[]{1, ny, nx}).reduce().
                        copyToNDJavaArray();
                
                NanoPhyto_integ_tp1 = (float[][]) ncIn.findVariable(strNanoPhyto_int).
                        read(origin, new int[]{1, ny, nx}).reduce().
                        copyToNDJavaArray();
                
                MicroZoo_integ_tp1 = (float[][]) ncIn.findVariable(strMicroZoo_int).
                        read(origin, new int[]{1, ny, nx}).reduce().
                        copyToNDJavaArray(); 
                
                MesoZoo_integ_tp1 = (float[][]) ncIn.findVariable(strMesoZoo_int).
                        read(origin, new int[]{1, ny, nx}).reduce().
                        copyToNDJavaArray();
        ...
        for (int ii = 0; ii <
            ...
            for (int jj = 0; jj <
                ...
                if (isInWater(i + ii, j + jj)) {
                    ...
                    double z_r = 0.d;
                    z_r =
                            z_rho_cst[k][j + jj][i + ii] + (double) zeta_tp0[j +
                            jj][i + ii] *
    ...
    int i, j, ii, jj;
    i = (int) (Math.round(xRho));
    j = (int) (Math.round(yRho));
    ii = (i - (int) xRho) == 0 ? 1 : -1;
    jj = (j - (int) yRho) == 0 ? 1 : -1;
    return !(isInWater(i + ii, j) && isInWater(i + ii, j + jj) &&
    isInWater(i, j + jj));
...
/*
    ...
    static boolean isCloseToCost(double[] pGrid) {
        ...
        int i, j;
        i =
                (int) (Math.floor(pGrid[0]));
        j =
                (int) (Math.floor(pGrid[1]));
 ...
 * */
        ...
        if (found) {
            ...
            while (((imax - imin) > 1) | ((jmax - jmin) > 1)) {
                if ((imax - imin) > 1) {
                    i0 = (imin + imax) / 2;
                    found =
                            isInsidePolygone(imin, i0, jmin, jmax, lon, lat);
                    if (found) {
                        imax = i0;
                    } else {
                        imin = i0;
                ...
                if ((jmax - jmin) > 1) {
                    j0 = (jmax + jmin) / 2;
                    found =
                            isInsidePolygone(imin, imax, jmin, j0, lon, lat);
                    if (found) {
                        jmax = j0;
                    } else {
                        jmin = j0;
            ...
            dx1 =
                    lonRho[jmin + 1][imin] - lonRho[jmin][imin];
            dy2 =
                    latRho[jmin][imin + 1] - latRho[jmin][imin];
            dx2 =
                    lonRho[jmin][imin + 1] - lonRho[jmin][imin];

            c1 =
                    lon * dy1 - lat * dx1;
            c2 =
                    lonRho[jmin][imin] * dy2 - latRho[jmin][imin] * dx2;
            deltax =
                    (c1 * dx2 - c2 * dx1) / (dx2 * dy1 - dy2 * dx1);
            deltax =
                    (deltax - lonRho[jmin][imin]) / dx2;
            xgrid =
                    (double) imin + Math.min(Math.max(0.d, deltax), 1.d);

            c1 =
                    lonRho[jmin][imin] * dy1 - latRho[jmin][imin] * dx1;
            c2 =
                    lon * dy2 - lat * dx2;
            deltay =
                    (c1 * dy2 - c2 * dy1) / (dx2 * dy1 - dy2 * dx1);
            deltay =
                    (deltay - latRho[jmin][imin]) / dy1;
            ygrid =
     ...
     * the domain.
     */
    public static double getTemperature(double xRho, double yRho, double zRho, double frac) throws
     ...
     * the domain.
     */
    public static double getSalinity(double[] pGrid, double frac) throws
            ArrayIndexOutOfBoundsException {
    ...
    public static double getHBL(double xRho, double yRho, double frac) throws
            ArrayIndexOutOfBoundsException {
        if (!FLAG_HBL) {
            return Double.NaN;
        }
            ...
            for (int ii = 0; ii < n; ii++) {
                {
                    ...
                    x = 0.d;
                    try {
                        x = (1.d - frac) * hbl_tp0[j + jj][i + ii] +
                                frac * hbl_tp1[j + jj][i + ii];
                        hbl += x * co;
                    } catch (ArrayIndexOutOfBoundsException e) {
                        throw new ArrayIndexOutOfBoundsException(
                                "Problem interpolating salinity field : " +
    ...
    public static double getZETA(double xRho, double yRho, double frac) throws
            ArrayIndexOutOfBoundsException {
        double co, CO, x, zeta;
        int n = isCloseToCost(xRho, yRho) ? 1 : 2;
        //-----------------------------------------------------------
            ...
            for (int ii = 0; ii < n; ii++) {
                {
                    ...
                    x = 0.d;
                    try {
                        x = (1.d - frac) * zeta_tp0[j + jj][i + ii] +
                                frac * zeta_tp1[j + jj][i + ii];
                        zeta += x * co;
                    } catch (ArrayIndexOutOfBoundsException e) {
                        throw new ArrayIndexOutOfBoundsException(
                                "Problem interpolating salinity field : " +
...
// ------------------------------------------------- //
    ...
    public static double[] get_uvw(double[] pGrid, double frac, int dt) throws
            ArrayIndexOutOfBoundsException {
                ...
                for (int jj = 0; jj <
                    ...
                    for (int kk = 0; kk <
                        ...
                        CO +=
                                co;
                        x_chiant =
                                (1.d - x_euler) * w_tp0[k + kk][j + jj][i + ii] + x_euler * w_tp1[k + kk][j + jj][i + ii];
                        dw +=
                                2.d * x_chiant * co /
                                (z_w_tp0[Math.min(k + kk + 1, nz)][j +
                                jj][i +
                                ii] - z_w_tp0[Math.max(k + kk - 1, 0)][j +
                                jj][i + ii]);
                    }
                    ...
                    for (int kk = 0; kk <
                        ...
                        //if (isInWater(i + ii, j + jj)) {
                        {
                            ...
                            CO +=
                                    co;
                            x_chiant =
                                    (1.d - x_euler) * u_tp0[k + kk][j + jj][i + ii -
                    ...
                    for (int ii = 0; ii <
                        ...
                        //if (isInWater(i + ii, j + jj)) {
                        {
                            ...
                            CO +=
                                    co;
                            x_chiant =
                                    (1.d - x_euler) * v_tp0[k + kk][j + jj - 1][i +
...
/*
        du *= Simulation.dt_advec * 60;//3600;
        dv *= Simulation.dt_advec * 60; //3600;
        dw *= Simulation.dt_advec * 60; //3600;
*/
// 6 avril 2014
        du *= dt * 60;//3600;
        dv *= dt * 60; //3600;
...
//        frac = (dt_HyMo - Math.abs(time_tp1 - time)) / dt_HyMo;
                ...
                for (int ii = 0; ii <
                    ...
                    if (isInWater(i + ii, j + jj)) {
                        ...
                        x =
                                0.d;
                        x =
                                (1.d - frac) * Diatoms_tp0[k + kk][j + jj][i + ii] + frac * Diatoms_tp1[k + kk][j + jj][i + ii];
                        Diatoms +=
                                x * co;

                        x =
                                (1.d - frac) * NanoPhyto_tp0[k + kk][j + jj][i + ii] + frac * NanoPhyto_tp1[k + kk][j + jj][i + ii];
                        NanoPhyto +=
                                x * co;

                        x =
                                (1.d - frac) * MicroZoo_tp0[k + kk][j + jj][i + ii] + frac * MicroZoo_tp1[k + kk][j + jj][i + ii];
                        MicroZoo +=
                                x * co;

                        x =
                                (1.d - frac) * MesoZoo_tp0[k + kk][j + jj][i + ii] + frac * MesoZoo_tp1[k + kk][j + jj][i + ii];
                        MesoZoo +=
                                x * co;

            //            x =
              //                  (1.d - frac) * O2_tp0[k + kk][j + jj][i + ii] + frac * O2_tp1[k + kk][j + jj][i + ii];
                //        O2 +=
                  //              x * co;
                    }
    ...
    // 6 nov 2016: onlit des sorties de PISCES dans les quelles phyto = phyto 1+ phyto2 et zoo = zoo1+zoo2
                ...
                for (int ii = 0; ii <
                    ...
                    if (isInWater(i + ii, j + jj)) {
                        ...
                        x =
                                0.d;
                        x =
                                (1.d - frac) * Phyto_tp0[k + kk][j + jj][i + ii] + frac * Phyto_tp1[k + kk][j + jj][i + ii];
                        Phytot +=
                                x * co;

                        x =
                                (1.d - frac) * Zoo_tp0[k + kk][j + jj][i + ii] + frac * Zoo_tp1[k + kk][j + jj][i + ii];
                        Zootot +=
                                x * co;
                    }
...
//        frac = (dt_HyMo - Math.abs(time_tp1 - time)) / dt_HyMo;
            ...
            for (int jj = 0; jj <
                ...
                for (int ii = 0; ii <
                    ...
                    if (isInWater(i + ii, j + jj)) {
                        ...
                        x =
                                (1.d - frac) * Det_tp0[k + kk][j + jj][i + ii] + frac * Det_tp1[k + kk][j + jj][i + ii];
                        det +=
                                x * co;

                        x =
                                (1.d - frac) * NO3_tp0[k + kk][j + jj][i + ii] + frac * NO3_tp1[k + kk][j + jj][i + ii];
                        NO3 +=
                                x * co;

                        x =
                                (1.d - frac) * Chla_tp0[k + kk][j + jj][i + ii] + frac * Chla_tp1[k + kk][j + jj][i + ii];
                        chla +=
                                x * co;
                    }
        ...
        if (!Simulation.flagRET_Chla_PISCES) {
                ...
                for (int ii = 0; ii <
                    ...
                    if (isInWater(i + ii, j + jj)) {
                        ...
                        x =
                                0.d;
                        x =
                                (1.d - frac) * DiatomsChl_tp0[k + kk][j + jj][i + ii] + frac * DiatomsChl_tp1[k + kk][j + jj][i + ii];
                        NanoPhytoChl +=
                                x * co;
                        x =
                                (1.d - frac) * NanoPhytoChl_tp0[k + kk][j + jj][i + ii] + frac * NanoPhytoChl_tp1[k + kk][j + jj][i + ii];
                        DiatomsChl +=
                                x * co;
                    }
                }
...
//EVOL2008 : 
            System.out.println("on ouvre le premier fichier netcdf (qui doit contenir les info de grille) pour en extraire les champs constants : "); // + listInputFiles.get(0));
//EVOL2009 :
//             String fileName = directory_roms + region + "_grd" + sufixe;       
//             System.out.println("on ouvre le fichier netcdf 'grille' pour en extraire les champs constants : " + fileName);
        }
...
//            System.out.print("POINT 2 Open dataset " + filename + "\n");
     ...
     *         <code>false</code> otherwise.
     */ // TIMOTHEE : ON THE EDGE OF LE DOMAIN QUe ON A SPECIFIER, RT NON PAS LA GRILLE ROMS EN ENTIER
    ...
    void readConstantField() throws IOException {
        ...
        try {
            ...
            arrPm =
                    ncIn.findVariable(strPm).read(origin, size);
            arrPn =
                    ncIn.findVariable(strPn).read(origin, size);
    ...
    void getCstSigLevels() throws IOException {
        ...
        try {
            if (ncIn.findGlobalAttribute(strThetaS) == null) {
                ...
                thetas =
                        ncIn.findVariable(strThetaS).readScalarDouble();
                thetab =
                        ncIn.findVariable(strThetaB).readScalarDouble();
                hc =
                        ncIn.findVariable(strHc).readScalarDouble();
            } else {
                System.out.println("ROMS UCLA");
                thetas =
                        (ncIn.findGlobalAttribute(strThetaS).getNumericValue()).doubleValue();
                thetab =
                        (ncIn.findGlobalAttribute(strThetaB).getNumericValue()).doubleValue();
                hc =
                        (ncIn.findGlobalAttribute(strHc).getNumericValue()).doubleValue();
            }
...
//        System.out.println("rank1 = " +rank);
        ...
        //System.out.println("Load data : jour = " +jour+ " , jourdumois = " +jourdumois+ " , month = " + month + " , rank " + rank + "...");
...
// 2) LIRE ET COPIER LES DONNEES du rang suivant dans "Tp1"
// (l'interpolation temporelle se fait entre Tp0 et Tp1, donc le jour doit être entre les deux rangs)
        rank += 1;
//        System.out.println("rank2 = " +rank);
    ...
    public static double[] getFields_SaltTemp(double xRho, double yRho, double zRho, double frac) throws IOException {
        ...
        salt =
                getSalinity(pGrid, frac);
        temp =
                getTemperature(xRho, yRho, zRho, frac);
        /*    if (temp < 10) {
        System.out.println("temp = " + temp);
        double[] CORRD = grid2Geo(xRho, yRho, zRho);
        System.out.println("double xRho, double yRho, double zRho, double frac =  " + xRho + " , " + yRho + " , " + zRho + " , " + frac);
        System.out.println("grid2Geo = " + CORRD[0] + " , " + CORRD[1] + " , " + CORRD[2]);
        System.out.println("getBathy --> BATHY = " + getBathy((int) xRho, (int) yRho));
        }
         */
        if (Simulation.lire_HBL) {
            hbl = getHBL(xRho, yRho, frac);
        } else {
            hbl = 0;
        }

        return (new double[]{temp, salt, hbl});
    }

    public static double[] getFields_uvw(double xRho, double yRho, double zRho, double frac, int dt) throws IOException {
    ...
    public static double[] get_uv(double[] pGrid, double frac, int dt) throws
            ArrayIndexOutOfBoundsException {
        //System.out.println("DANS get_uvw :  w_tp1.length = " + w_tp1.length);
            ...
            for (int ii = 0; ii < n; ii++) {
                {
                    ...
                    x = 0.d;
                    try {
                        x = (1.d - frac) * Diatoms_integ_tp0[j + jj][i + ii] +
                                frac * Diatoms_integ_tp1[j + jj][i + ii];
                        Diatoms += x * co;
                    } catch (ArrayIndexOutOfBoundsException e) {
                        throw new ArrayIndexOutOfBoundsException(
                                "Problem interpolating salinity field : " +
            ...
            for (int ii = 0; ii < n; ii++) {
                {
                    ...
                    x = 0.d;
                    try {
                        x = (1.d - frac) * MesoZoo_integ_tp0[j + jj][i + ii] +
                                frac * MesoZoo_integ_tp1[j + jj][i + ii];
                        MesoZoo += x * co;
                    } catch (ArrayIndexOutOfBoundsException e) {
                        throw new ArrayIndexOutOfBoundsException(
                                "Problem interpolating salinity field : " +

...
// --------- DECLARATION DES VARIABLES -----------------------------------------
    ...
    // Range de tolerances :
    public static float rayon_exploration_spatial_max, rayon_exploration_spatial_min,
            rayon_exploration_temporel_max, rayon_exploration_temporel_min,
            Range_tolerance_temperature, Range_tolerance_salinite,
            Range_tolerance_batymetrie, Range_tolerance_HBL;
    ...
    public static int egg_duration, YolkSacLarvae_duration, nb_record;
    ...
    public static boolean flagRET_Chla_PISCES, flagGREG_dens, flagRET_shelf,
            flagRET_Chla_SeaWiFS, BetHedging, flagSwim_O2, flagSwim_random,
            record_initial_env, flagBuoy, flagGrowth, flagTurbulence, lire_HBL,
            flag_Densite_max, Ponte_Chla_SeaWiFS;
    public static boolean flagRET_CarbZ_PISCES, outpout_netcdf_flag, TEST_KINESIS,
            flag_sigma_t_swim_evolutif, flag_swim_temp_natal, Flag_pref_thermique_nage,
    ...
    // OPPORTUNISME
    // cible
    static double Topp, Sopp, bathymax_opp, bathymin_opp, Th_opp, DensOpp,
            F_annuel, Min_fishing_size, food_half_saturation, carcapa_max_km2_shelf;
    // marge (=tolerances)
...
// E - CRITERES PHYSIQUES AFECTANT LE RECRUTEMENT
            ...
            System.out.println("flag_Densite_max = " + flag_Densite_max);

// XX - PARAMETRES DE LA STRATEGIE DE NAGE 
            // sigma_t = largeur de la gaussienne autour de la temperature cible de nage (temperature natale)
            // Soit elle croit avec l'age
            // soit elle decroit
            // soit c'est un parametre constant tout au long de la vie mais hereditaire
            // --> cette derniere solution est appliquee si flag_sigma_t_swim_evolutif = true
            // on pourra eventuelement mettre en evidence des strategies de migration differentes
...
// D - PARAMETRES DE LA STRATEGIE DE PONTE
            ...
            System.out.println("vertical_patchiness_rad = " + vertical_patchiness_rad);

            patchiness_rad_OPP = Dataset_EVOL.param_config.getConfig_int("patchiness_rad_OPP");     // [Kilometres]        OPPPORTUNISM
            System.out.println("patchiness_rad_OPP = " + patchiness_rad_OPP);
            ...
            System.out.println("resolution_temporelle_scanEnv = " + resolution_temporelle_scanEnv);
            ...
            YolkSacLarvae_duration = Dataset_EVOL.param_config.getConfig_int("YolkSacLarvae_duration"); // [jours] durée du stade oeuf
            ...
            Vitesse_max_Bls = Dataset_EVOL.param_config.getConfig_double("Vitesse_max_Bls"); // Vitesse max en body lenght par seconde (QUI PEUT ETRE TENUE PENDANT UN PAS DE TEMPS)
            delta_kinesis = Dataset_EVOL.param_config.getConfig_double("delta_kinesis"); //pondere growth and local_mortality dans Q : 0-> tout sur Growth; 1-> tout sur mortalite
            // F - PECHE
...
// G - DIVERS
            dt_output = Dataset_EVOL.param_config.getConfig_int("dt_output"); // intervalle d'enregistrement des sorties [Jours]
            ...
            nbeggs_max_per_SI = Dataset_EVOL.param_config.getConfig_double("nbeggs_max_per_SI"); //1e13;//1e10; //14;
            nrec_max_jour = (float) Dataset_EVOL.param_config.getConfig_double("nrec_max_jour");

            Flag_pref_thermique_nage = Dataset_EVOL.param_config.getConfig_boolean("Flag_pref_thermique_nage"); // true : temperature joue un rôle dans indice de qualité d'habitat. false : ne joue aucun role.
            flag_swim_temp_natal = Dataset_EVOL.param_config.getConfig_boolean("flag_swim_temp_natal"); // trie = pour le HQI, le It se base sur la temperature de naissance. false : se base sur la literature: Topt = 21, sigma=3
            Flag_pref_bouffe_nage = Dataset_EVOL.param_config.getConfig_boolean("Flag_pref_bouffe_nage"); // true : bouffe joue un rôle dans indice de qualité d'habitat. false : ne joue aucun role.
            ADVECTION = Dataset_EVOL.param_config.getConfig_boolean("ADVECTION"); // false : enlève l'advection des courants 3D pour les adultes
            FLAG_PLANKTON_INTEG = Dataset_EVOL.param_config.getConfig_boolean("FLAG_PLANKTON_INTEG"); // pour lire le plankton integre au lieu du 3D
            food_half_saturation = Dataset_EVOL.param_config.getConfig_double("food_half_saturation");
            carcapa_max_km2_shelf = Dataset_EVOL.param_config.getConfig_double("carcapa_max_km2_shelf");
...
// TODO Auto-generated catch block
        ...
        FLAG_KinesisDD = false; // : pour prendre en compte la densite dependance dans les mouvement de kinesis
...
//---------------------------------------------------------------------------------------------------------------
    public void PonteInitiale() {
        ...
        if (outpout_netcdf_flag) {
            ...
            try {
                netcdf_outpout_writer.openTrajIndiv_NetCDF();
...
// DEBUG : stat de sorties des adultes
            ...
            System.out.println(" STARVED > 5cm : " + Population.starved);
...
//        ss.append("rayon_exploHE (en Km): " + rayon_exploHE + "\n");
        ...
        s.append("Flag_pref_thermique_nage = " + Flag_pref_thermique_nage + "\n");
        ...
        s.append("nbeggs_max_per_SI : " + nbeggs_max_per_SI + "\n");
        s.append("nrec_max_jour : " + nrec_max_jour + "\n");
        s.append("delta_kinesis : " + delta_kinesis + "\n");
        s.append("FLAG_PLANKTON_INTEG : " + FLAG_PLANKTON_INTEG + "\n");
        s.append("food_half_saturation : " + food_half_saturation + "\n");
...
// -----------------------------------------------------------------------------
    ...
    public void compress() {
        ...
        String filename_traj = output_dir + Strategie + "_TrajIndiv_Y" + compteur_year + ".nc";
        String filename_Biom2D = output_dir + Strategie + "_Biom2D_Y" + compteur_year + ".nc";
        gZip.gzipIt(filename_traj);
        gZip.gzipIt(filename_Biom2D);
                
        File file1 = new File(filename_traj);
        file1.delete();
        File file2 = new File(filename_Biom2D);
        file2.delete();

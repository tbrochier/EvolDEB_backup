...
// ------------------------------------------------------------------------------------------------------
    ...
    int[] liste_d;
    static int sortie_NORD, sortie_SUD, sortie_EST, sortie_OUEST, starved, starved_larvae;
    ...
    public static float[] N_recmax_SI, Surf_ZONES;
    public static int[] N_rec_SI;
    public static int[] Biom_zone;
    int nrec_jour;
    int compteur_larves, compteur_juveniles, compteur_adultes;
...
//                    System.out.println("Poisson " + ss + " début...");
                ...
                if ((Simulation.TEST_KINESIS == true) && (Simulation.RECORD_TRACE == true)) {
                    System.out.println("compteur_advec =  " + Po.compteur_advec);
                }
...
//                Po.living = Po.DEB. A VOIR
                    if (Po.living == false) {
                        ...
                        if ((Simulation.TEST_KINESIS == true) && (Simulation.RECORD_TRACE == true)) {
                            System.out.println("Poisson mort: " + Po.Cause_de_la_mort);
                            try {
                                Po.Sim_writer_trace.CloseTRACE_NetCDF();
                            } catch (Exception ex) {
                ...
                Biom_zone[Po.zone] += (int) Po.S * Po.weight;
...
//+ AJOUTER LE TRUC "DEUXIEME PASSAGE" <--- ???
            if ((jour % Simulation.dt_output) == 0) {
                ...
                if (Simulation.outpout_netcdf_flag) {
                    ...
                    try {
                        Sim_writer.write_TrajIndiv_NetCDF(Pop);
                    } catch (Exception ex) {
                        System.out.println("probleme dans write_TrajIndiv_NetCDF(Pop) : " + ex.getMessage());
                    }
                    try {
                        Sim_writer.write_Biom2D_NetCDF();
                    } catch (Exception ex) {
                        System.out.println("probleme dans write_Biom2D_NetCDF : " + ex.getMessage());
                    }                }
...
//                compteur_id_poissons++;
        ...
        } else { // (TEST_KINESIS==true)
            jour_debut_ponte = 0;
            jour_fin_ponte = jour_debut_ponte + 1;
            for (int jour = jour_debut_ponte; jour
                    < jour_fin_ponte;
                    jour++) {
                for (int p = 0; p < Simulation.Pop_max; p++) { // Ici "Simulation.nbFishes_pasdetemps" a été remplacé par "Simulation.nbFishesJour" le 21 mars 2013
                    Poissons Po = new Poissons(jour, Sim_writer);
                    Pop.add(Po);
                }
            }
...
//                            compteur_id_poissons++;
        ...
        for (p = 0; p
            ...
            if (Simulation.record_initial_env && (Po.day_init > day_init_temp
                ...
                try {
                    // ATTENTION : A VOIR LE BORDEL AVEC LES CHGMENTS FICHEIRS NCML
...
// MORTALITE INITIALE = 9/10      
                    if (Po.bathyActuelle > -200) {
                        nb_nouveaux_super_indiv = (int) Math.floor(nb_nouveaux_super_indiv);
                    } else {
                        nb_nouveaux_super_indiv = 1;
                    }

                    //                   if (nb_nouveaux_super_indiv>3){
                    //                     nb_nouveaux_super_indiv = 4;
                    //                }
                    // fin test
                    //         System.out.println(" SI de  = " + Po.Biomasse_SI/1e6 + " tonnes qui pond " + nb_nouveaux_super_indiv + " SI oeufs ");
//        System.out.println("nbponte_par_indiv  = " + Simulation.nbponte_par_indiv);
//        System.out.println("Po.Nb_strates = " + Po.Nb_strates);
//        System.out.println("N_ponte_pour_fermeture_pop = " + N_ponte_pour_fermeture_pop);
                    // L'effectif S des nouveau super individu pondu est :
                    S_newSI = (int) Math.min(Simulation.nbeggs_max_per_SI, NEggs); //Math.round(NEggs / nb_nouveaux_super_indiv); corr juin 2014
...
// NE MARCHE PAS
        ...
        nb_oeuf = (int) Math.floor(Simulation.nbponte_par_indiv * fn) + 1;
        return nb_oeuf;
    }
...
// on veut un recrument max de Simulation.nrec_max_jour individu par jour
// donc il faut un recmax de (Simulation.nrec_max_jour/surf_plateau_nbmaille)*surface_zone dans chaque zone
...
//    N_recmax_SI[zone]  = (int) (Math.floor(surf_grd_zone_i*(Simulation.nrec_max_jour/((float) surf_plateau_nbmaille))));
            N_recmax_SI[zone] = surf_grd_zone_i * (Simulation.nrec_max_jour / ((float) surf_plateau_nbmaille));

            Surf_ZONES[zone] = surf_grd_zone_i * mean_grid_size * mean_grid_size;
            //System.out.println("Zone " + zone + "  --> N_recmax_SI = " + N_recmax_SI[zone]);   
            System.out.println("Zone " + zone + " : " + Math.round(Dataset_EVOL.lat_min + zone) + "-" + Math.round(Dataset_EVOL.lat_min + zone + 1) + "°N  surface du plateau = " + Math.round(Surf_ZONES[zone]) + "km2");
